...
- **Game 1** — устоявшийся шаблон (не свободный выбор между двумя стилями):
  ...
  предыдущих уроков дословно. Эталон: "Hen, Hen, what do we do?" →
  "Jump, jump, jump, me and you!" / "Run, run, run, she is coming for you!".
  Грамматику рифм всегда проверять. НЕ привязывать команды к самой LEGO-
  модели буквально (см. правило про зонтик).
...
- **Game 3**: обязательно использует построенную модель физически, той же
  ...
  случайно выбирает 1 из 2 ответов), СЛОВА новые под тему. Нужны ДВЕ явно
  различающиеся команды учителя с понятной причиной, связанной с темой
  (например "sunny day, slither slow!" / "storm is coming, slither fast!") —
  не просто "беги к дереву" без объяснения. Финальная `action`/`teacher`
  строка — прямой мостик в Challenge (но САМА демонстрация решения Challenge
...
Для большинства промптов: полу-реалистичный дружелюбный стиль (НЕ мультяшный
флэт, кроме исключений вроде солнца/абстрактных природных объектов), без
текста на картинке, фон истории — ГОРИЗОНТАЛЬНЫЙ.

**Исключение — `real_object_image_prompt`**: это НАСТОЯЩАЯ фотография
объекта темы (не LEGO, не иллюстрация) — формулировать как "A real
photograph of an actual [object], photorealistic, natural lighting, simple
clean background, no text". Это единственное поле, где нужен фотореализм,
а не полу-реалистичная иллюстрация.
